...
public class LoggingAspect {
    ...
    @Before("execution(* com.example.lab4.controller.*.*(..))")
    public void logBeforeController(JoinPoint joinPoint) {
        logger.info("Entering controller method: {} with arguments: {}",
                joinPoint.getSignature().getName(), Arrays.toString(joinPoint.getArgs()));
    }

    @AfterReturning(pointcut = "execution(* com.example.lab4.controller.*.*(..))", returning = "result")
    public void logAfterController(JoinPoint joinPoint, Object result) {
        logger.info("Exiting controller method: {} with result: {}",
                joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.example.lab4.controller.*.*(..))", throwing = "exception")
    ...
    @AfterReturning(pointcut = "execution(* com.example.lab4.service.*.*(..))", returning = "result")
    public void logAfterService(JoinPoint joinPoint, Object result) {
        logger.debug("Exiting service method: {} with result: {}",
                joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.example.lab4.service.*.*(..))", throwing = "exception")
